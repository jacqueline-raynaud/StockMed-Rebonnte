...
### T-01 · Initialiser l'historique git · 0,1 j

**Problème.** l'application récupérée ne buid pas en cause JVM 25

**Fait.** Récupérer le code, créer un nouveau dépôt, builder

### T-02 · Moderniser la chaîne de build · 0,25 j
...
### T-04 · Le stock fermait l'application · 0,25 j
...
**Fait.** L'opération est descendue dans le repository - le viewModel gère le calcul:  
`updateStock(id, delta, userEmail)` cible le médicament désigné et borne le stock  
à zéro.

**Détail.** Normalement cela n'a pas été commité. Pour pouvoir tester l'application en   
l'état une correction avec `size - 1` supprime le plantage mais visé le  
dernier médicament de la liste au lieu de celui affiché. La corruption de l'historique  
est volontaire, juste le temps de pouvoir continuer à tester.

### T-05 · L'historique n'était jamais enregistré · 0,25 j
...
### T-20 · Journalisation systématique · 0,75 j
...
### T-22 · Tri et filtre côté serveur · 0,5 j {#t-22}
...
-	Utilisation d'une documentation à l'aide d'un projet Github Page avec une action dédiée. 
-	Dans le repo Github, paramétrer pour avoir un déploiement sur une action se déclenchant en push main ou pull request main
...
### T-34 · Externalisation des chaînes · 0,1 j
...
**Demande : ** 
- Déplacer et renommer les anciens repository (tounent en local) dans test
...
Les previews portent sur : les quatre erreurs de saisie ensemble, une base sans  
...
Un  fichier pour les codes d'erreur de Firestore. Au-dessus, tout le  
...
*   Les **transactions** Firestore, dont dépendent les mouvements de stock, ne  
    fonctionnent pas hors ligne. Laisser les boutons actifs permettrait des  
...
**L'historique n'est pas touché**, et l'opérateur en est averti _avant_ de  
valider. Voir la [question ouverte](#rgpd) sur le sort de cette donnée  
personnelle.

### T-21 · Modification du stock contrôle · 0,5 j

**Problème.** Le retrait d'article doit être plafonné au nombre en stock
...
**Décision - le stock n'est pas modifiable ici.**   
Corriger une quantité contournerait la traçabilité.  
...
**Pourquoi.** Deux « Stockage froid » seraient **indiscernables dans**  
...
**Pourquoi.** Sur deux cents références dont douze au froid, ouvrir  
...
_dans le code_ du livrable « fuites mémoire » : un lecteur qui ouvre  
...
signale rien : son détecteur _plante_. Le lint de Compose fourni par le BOM  
